...
impl<'tcx> TyCtxt<'tcx> {
    pub fn has_error_field(self, ty: Ty<'tcx>) -> bool {
        if let ty::Adt(def, substs) = *ty.kind(self) {
            for field in def.all_fields() {
                let field_ty = field.ty(self, substs);
                if let Error(_) = field_ty.kind(self) {
                    return true;
                }
            }
        }
    ...
    pub fn destructor_constraints(self, def: &'tcx ty::AdtDef) -> Vec<ty::subst::GenericArg<'tcx>> {
        ...
        let impl_substs = match *self.type_of(impl_def_id).kind(self) {
            ty::Adt(def_, substs) if def_ == def => substs,
            _ => bug!(),
        };

        let item_substs = match *self.type_of(def.did).kind(self) {
            ty::Adt(def_, substs) if def_ == def => substs,
            _ => bug!(),
        };
...
impl<'tcx> ty::TyS<'tcx> {
    ...
    fn is_trivially_freeze(&self, tcx: TyCtxt<'tcx>) -> bool {
        match self.kind(tcx) {
            ...
            | ty::Error(_)
            | ty::FnPtr(_) => true,
            ty::Tuple(_) => self.tuple_fields(tcx).all(|f| f.is_trivially_freeze(tcx)),
    ...
    pub fn needs_drop(&'tcx self, tcx: TyCtxt<'tcx>, param_env: ty::ParamEnv<'tcx>) -> bool {
        ...
        match needs_drop_components(self, tcx) {
    ...
    pub fn same_type(a: Ty<'tcx>, b: Ty<'tcx>, tcx: TyCtxt<'tcx>) -> bool {
        match (&a.kind(tcx), &b.kind(tcx)) {
            (&Adt(did_a, substs_a), &Adt(did_b, substs_b)) => {
                ...
                substs_a.types().zip(substs_b.types()).all(|(a, b)| Self::same_type(a, b, tcx))
            }
            _ => a == b,
        }
    }
                ...
                Adt(def, _) => {
                    {
                        ...
                        if let Some(&seen_type) = iter.next() {
                            if same_struct_or_enum(seen_type, *def, tcx) {
                        ...
                        for &seen_type in iter {
                            if ty::TyS::same_type(ty, seen_type, tcx) {
    ...
    ///
    /// This method is idempotent, i.e. `ty.peel_refs(tcx).peel_refs(tcx) == ty.peel_refs(tcx)`.
    ///
    /// # Examples
    ///
    /// - `u8` -> `u8`
...
pub fn needs_drop_components(
    ...
    match ty.kind(tcx) {
        ...
        ty::Slice(ty) => needs_drop_components(ty, tcx),
        ty::Array(elem_ty, size) => {
            match needs_drop_components(elem_ty, tcx) {
                Ok(v) if v.is_empty() => Ok(v),
                res => match size.val.try_to_bits(tcx.data_layout.pointer_size) {
        ...
        ty::Tuple(..) => ty.tuple_fields(tcx).try_fold(SmallVec::new(), move |mut acc, elem| {
            acc.extend(needs_drop_components(elem, tcx)?);
            Ok(acc)
        }),
